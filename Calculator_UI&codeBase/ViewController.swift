...
class ViewController: UIViewController {
    ...
    @objc func clickNum(_ sender: UIButton) {
        ...
        if currentText == "0" && clickedButtonText != "0" {
            ...
            currentText = removeZero(expression: currentText + clickedButtonText)
        }
        updateLabel()
    }
    ...
    func removeZero(expression: String) -> String {
        var result = ""
        var opertorTrack = false

        for char in expression {

            if char.isNumber {          // 숫자일 때
                // char가 0이아닌 숫자이거나 연산자 뒤가 아니면 result에 추가
                if char != "0" || !opertorTrack {
                    result.append(char)
                    opertorTrack = false
                }
            } else {                    // 숫자가 아닐 때
